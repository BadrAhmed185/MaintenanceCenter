document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadItems() {
        try {
            const response = await ApiClient.get('/maintenanceservices');
    ...
    window.deleteItem = async function (id) {
        if (confirm('هل أنت متأكد من حذف هذا العنصر؟')) {
            try {
                await ApiClient.delete(`/maintenanceservices/${id}`);
    ...
    catalogForm.addEventListener('submit', async function (e) {
        ...
        try {
            if (id) {
                data.id = parseInt(id);
                await ApiClient.put(`/maintenanceservices/${id}`, data);
            } else {
                await ApiClient.post('/maintenanceservices', data);
            }
